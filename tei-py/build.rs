...
//! produced `cdylib` does not link `libpython`. During `cargo test` the signal
//! is absent, so the script instead emits `cargo:rustc-link-*` directives
//! pointing at the host interpreter, allowing `PyO3` to link successfully.
