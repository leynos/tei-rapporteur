...
#: mutable workspace crates, excludes for test scaffolding and
#: generator binaries, and feature arguments matching the CI baseline.
EXPECTED_WITH = {
    "paths": "tei-core/,tei-serde/,tei-xml/,tei-py/",
    "exclude-globs": (
        "tei-test-helpers/**,"
        "tei-py/src/bindings_test_support.rs,"
        "tei-py/src/test_support/**,"
        "tei-py/src/test_support_tests/**,"
        "tei-xml/src/fixtures/**,"
        "tei-xml/src/streaming/helpers_tests.rs,"
        "tei-xml/src/bin/**,"
        "tei-serde/src/bin/**,"
        "tei-xml/examples/**"
    ),
...
def test_triggers_keep_schedule_and_plain_dispatch() -> None:
    ...
    assert schedule == [{"cron": "5 9 * * *"}], (
        f"on.schedule must be the daily 09:05 UTC cron, got {schedule!r}"
    )
...
def test_with_block_carries_the_caller_configuration() -> None:
    ...
    assert with_block == EXPECTED_WITH, (
        "jobs.mutation.with must equal the documented caller configuration "
